...
if __name__ == '__main__':
    ...
    parser.add_argument('--exextra_view', default=0, type=int, help="默认视图中额外多加了90和270")
    parser.add_argument('--bg_color', default=0, type=int, help="0黑1白")
    parser.add_argument('--mask_use_step', default=8000, type=int, help="更新mask后，mask的最大使用期限")
    parser.add_argument('--unletter', default=0, type=int, help="删除了")
    parser.add_argument('--update_mask_epoch', default=500, type=int, help="到达该次数时，更新mask路径，更新mask_stop_step=mask_use_step，继续使用mask")
    parser.add_argument('--mask_stop_step', default=5000, type=int, help="迭代次数大于该值就停止")
    parser.add_argument('--mask_image', default='', type=str, help="mask_image")
    ...
    parser.add_argument('--test_interval', type=int, default=10, help="test on the test set every interval epochs")
    ...
    parser.add_argument('--known_view_interval', type=int, default=24, help="train default view with RGB loss every & iters, only valid if --image is not None.")

    parser.add_argument('--IF', action='store_true', help="experimental: use DeepFloyd IF as the guidance model for nerf stage")

    parser.add_argument('--guidance', type=str, nargs='*', default=['IF'], help='guidance model')
    ...
    parser.add_argument('--sd_version', type=str, default='2.1', choices=['1.5', '2.0', '2.1', 'mv'], help="stable diffusion version")
    ...
    if opt.six_views:
        ...
        if 'MVDream' in opt.guidance:
            from guidance.sd_utils import MVDream
        ...
        # 数据的预处理，优化器配置，文件夹目录配置，准备text embdding。模型的具体训练，测试，损失函数计算，梯度更新，保存网格都在这。
        ...
        trainer.default_view_data = train_loader._data.get_default_view_data(extra_view=opt.exextra_view)
        # gui 一般测试的时候才会用
        if opt.gui:
            from nerf.gui import NeRFGUI
