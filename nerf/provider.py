...
class NeRFDataset:
    ...
    def get_default_view_data(self, extra_view):
        ####################################################################################################################################################################
        H = 64
        W = 64
        cx = H / 2
        cy = W / 2
        default_view = [i*45 for i in range(360//45)]
        # default_view = [i * 90 for i in range(360 // 90)]
        if extra_view:
            default_view.append(90)
            default_view.append(270)
        datas = []
        for i in default_view:
            radii = torch.FloatTensor([3.2]).to(self.device)
            thetas = torch.FloatTensor([90]).to(self.device)
            phis = torch.FloatTensor([i]).to(self.device)
            poses, dirs = circle_poses(self.device, radius=radii, theta=thetas, phi=phis, return_dirs=True, angle_overhead=30, angle_front=60)
            fov = 20
            focal = H / (2 * np.tan(np.deg2rad(fov) / 2))
            intrinsics = np.array([focal, focal, cx, cy])
            self.far, self.near = 1000, 0.01
    ...
    def collate(self, index):
        ...
        data = {
            'H': self.H,  # 图像高度
            'W': self.W,  # 图像宽度
            'rays_o': rays['rays_o'],  # 射线原点
            'rays_d': rays['rays_d'],  # 射线方向单位向量
            'dir': dirs,  # 从图片坐标系转换到相机坐标系，也是二维坐标
            'mvp': mvp,  # model-view-project
            'polar': delta_polar,  # 极坐标下，射线的角度，即和z轴的角度
            'azimuth': delta_azimuth,  # 方位角
            'radius': delta_radius,  # 半径，即射线长度
        }
