...
def remove_jagged_edges(image_path):
    # 读取图像
    ...
    # 再次二值化，以恢复图像清晰度
    _, final_image = cv2.threshold(blurred_image, 128, 255, cv2.THRESH_BINARY)
...
def get_pos_neg_text_embeddings(embeddings, azimuth_val, opt):
    if azimuth_val >= -90 and azimuth_val < 90:  # 这个角度被认为是正视图和侧视图（-90，90）， 对照着assert这个图去看
        if azimuth_val >= 0:
            r = 1 - azimuth_val / 90
        else:
            r = 1 + azimuth_val / 90
        ...
        pos_z = r * start_z + (1 - r) * end_z  # 如果没有perpneg,pos_z就是最后的text_z,
        text_z = torch.cat([pos_z, embeddings['front'], embeddings['side']], dim=0)  # perpneg额外加的一步是把其他两个视图加进来，然后生成一个权重，用于削弱前视图和侧视图
        if r > 0.8:
            front_neg_w = 0.0
        else:
            front_neg_w = math.exp(-r * opt.front_decay_factor) * opt.negative_w
        if r < 0.2:
            side_neg_w = 0.0  # 如果不是太侧，就基本是正视图，侧视图的权重由下公式计算
        else:
            side_neg_w = math.exp(-(1-r) * opt.side_decay_factor) * opt.negative_w  # 如果侧的太多，那就需要一些侧视图的权重

        weights = torch.tensor([1.0, front_neg_w, side_neg_w])
    else:
        if azimuth_val >= 0:  # （90，180）
        ...
        # if random.random() < 0.3:
        #     r = r + random.gauss(0, 0.08)
        pos_z = r * start_z + (1 - r) * end_z  # 当视角为180时，也就是-180，那就是纯后视图观察，r=0, 全是后视图的embdding
        text_z = torch.cat([pos_z, embeddings['side'], embeddings['front']], dim=0)
        front_neg_w = opt.negative_w 
        if r > 0.8:
            side_neg_w = 0.0
        else:
            side_neg_w = math.exp(-r * opt.side_decay_factor) * opt.negative_w / 2  # 如果不是太后视图，那就需要有一个侧视图的权重

        weights = torch.tensor([1.0, side_neg_w, front_neg_w])  # 最后返回一个权重，无条件编码，侧视图的权重，正视图的权重（如果是后视图，该值为负）
...
class Trainer(object):
    ...
    def prepare_embeddings(self):
        ...
        if self.opt.text is not None:
            ...
            if 'IF' in self.guidance:
                self.embeddings['IF']['default'] = self.guidance['IF'].get_text_embeds([self.opt.text])  # 文本提示词
                self.embeddings['IF']['uncond'] = self.guidance['IF'].get_text_embeds([self.opt.negative])  # 空

                for d in ['front', 'side', 'back']:
                    self.embeddings['IF'][d] = self.guidance['IF'].get_text_embeds([f"{self.opt.text}, {d} view"])  # 对这句话进行了补充，三种
    ...
    def train_step(self, use_default_views, data, save_guidance_path:Path=None):
        """
            Args:
        ...
        """
        # data数据在loader中已经加载好了，下面那么多代码是为了求bg_color，ambient_ratio，shading，binarize
        do_rgbd_loss = use_default_views
        if do_rgbd_loss:
            mask_image = self.opt.mask_image + '-' + str(data['azimuth'][0]) + '.png'
        else:
            mask_image = None
        ...
        if self.opt.progressive_view:  # 0 90 3.2 20
        ...
        if do_rgbd_loss:
            ...
            if self.opt.known_view_noise_scale > 0:  # 每次相机参数会存在一些误差  需要
        ...
        # 输出loss_orient, weights, image, 深度图，weights_sum（64X64的mask）;
        outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=False, perturb=True, bg_color=bg_color, ambient_ratio=ambient_ratio, shading=shading, binarize=binarize, t=self.global_step)
        pred_depth = outputs['depth'].reshape(B, 1, H, W)
        pred_mask = outputs['weights_sum'].reshape(B, 1, H, W)
        ####################################################################################################################
        #if self.local_step == 1:
        #   pred_rgb = outputs['image'].reshape(B, 3, H, W)
        #   torchvision.utils.save_image(pred_rgb, 'RGB_mask_A/pred_rgb_{}_{}_{}.png'.format(self.global_step, data['polar'].item(), data['azimuth'].item()), padding=0)
            # print(data['polar'])
            # torchvision.utils.save_image(pred_depth, 'depth_mask_A/pred_depth_{}_{}_{}.png'.format(self.global_step, data['polar'].item(), data['azimuth'].item()), padding=0)
            # torchvision.utils.save_image(pred_mask, 'depth_mask_A/pred_mask_{}_{}_{}.png'.format(self.global_step, data['polar'].item(), data['azimuth'].item()), padding=0)
        ####################################################################################################################
        ...
        if do_rgbd_loss:
            ...
            if self.opt.image is None:
                it = transforms.Compose([transforms.ToTensor()])
                gt_mask = Image.open(mask_image).resize([64, 64]).convert('L')
                gt_mask = it(gt_mask).to(self.device)
                loss = self.opt.lambda_mask * F.mse_loss(pred_mask[:, 0], gt_mask.float())
                if self.opt.update_mask_epoch < self.epoch:

                    mask_image2 = self.old_mask_image + '-' + str(data['azimuth'][0]) + '.png'

                    gt_mask2 = Image.open(mask_image2).resize([64, 64]).convert('L')
                    gt_mask2 = it(gt_mask2).to(self.device)
                    loss2 = self.opt.lambda_mask * F.mse_loss(pred_mask[:, 0], gt_mask2.float())
                    a = 0
                    if self.epoch > 40:
                        a = 0.4
                    if self.epoch > 50:
        ...
        # novel view loss
            ...
            if 'IF' in self.guidance:
                ...
                # ENHANCE: remove loop to handle batch size > 1
                text_z = [self.embeddings['IF']['uncond']] * azimuth.shape[0]  # 不知道为啥乘方位角
                if self.opt.perpneg:
                    text_z_comp, weights = adjust_text_embeddings(self.embeddings['IF'], azimuth, self.opt)  # 返回经过加权的embdding以及权重
                    text_z.append(text_z_comp)
                elif self.opt.unletter:
                    text_z.append(self.embeddings['IF']['default'])
                else:
                    for b in range(azimuth.shape[0]):
                        if azimuth[b] >= -90 and azimuth[b] < 90:
                            if azimuth[b] >= 0:
                                ...
                                r = 1 + azimuth[b] / 90
                            start_z = self.embeddings['IF']['front']  # a lion bust, front view
                            end_z = self.embeddings['IF']['side']  # a lion bust, side view
                        else:
                            if azimuth[b] >= 0:
                                r = 1 - (azimuth[b] - 90) / 90
                            else:
        ...
        if not self.opt.dmtet:
            # Try increasing/decreasing lambda_sparsity if your scene is stuffed with floaters/becoming empty.
            ...
            if self.opt.lambda_entropy > 0:
                ...
                loss_entropy = (- alphas * torch.log2(alphas) - (1 - alphas) * torch.log2(1 - alphas)).mean() # -[P(A)log(P(A)) + P(B)log(P(B))]
            ...
            if self.opt.lambda_orient > 0 and 'loss_orient' in outputs:  # loss_orient目前还没搞清楚这个
    ...
    def eval_step(self, data):
        with torch.no_grad():
            ...
            outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=True, perturb=False, bg_color=self.opt.bg_color, light_d=light_d, ambient_ratio=ambient_ratio, shading=shading, t=self.global_step)  # shading albedo ambient_ratio 1.0
            ...
            #####################################################
            if self.epoch >= 700:
                outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=False, perturb=True, bg_color=self.opt.bg_color, ambient_ratio=0.5, shading='lambertian', binarize=False, t=self.global_step, normal=True)
                pred_normal = outputs['normal_image'].reshape(B, H, W, 3)
            else:
                pred_normal = None
            #################################################################################333333

        return pred_rgb, pred_normal, pred_mask, loss
    ...
    def test_step(self, data, bg_color=None, perturb=False):
        ...
        outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=True, perturb=perturb, light_d=light_d,
                                    ambient_ratio=ambient_ratio, shading=shading, bg_color=self.opt.bg_color)
        ...
        #####################################################
        if self.epoch >= 700:
            outputs = self.model.render(rays_o, rays_d, mvp, H, W, staged=False, perturb=True, bg_color=self.opt.bg_color,
                                        ambient_ratio=0.5, shading='lambertian', binarize=False, t=self.global_step,
                                        normal=True)
            pred_normal = outputs['normal_image'].reshape(B, H, W, 3)
        else:
            pred_normal = None
        #########pred_normal########################################################################333333
        return pred_rgb, pred_normal, pred_mask, None
    ...
    def train(self, train_loader, valid_loader, test_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            # if epoch > self.opt.update_mask_epoch:
            #     self.opt.lambda_mask = 100
            #     print('----------------修改lambda_mask为100----------------')
            #     print('----------------修改lambda_mask为100----------------')
            #     print('----------------修改lambda_mask为100----------------')
            # if epoch > 100:
            #     self.opt.lambda_mask = 10
            #     print('----------------修改lambda_mask为10----------------')
            #     print('----------------修改lambda_mask为10----------------')
            #     print('----------------修改lambda_mask为10----------------')
            # if epoch > 150:
            #     self.opt.lambda_mask = 1
            #     print('----------------修改lambda_mask为1----------------')
            #     print('----------------修改lambda_mask为1----------------')
            #     print('----------------修改lambda_mask为1----------------')
            # if epoch > 200:
            #     self.opt.mask_stop_step = 0
            #     print('----------------修改mask_stop_step为0----------------')
            #     print('----------------修改mask_stop_step为0----------------')
            #     print('----------------修改mask_stop_step为0----------------')
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth, pred_mask, _ = self.test_step(data)  #  和验证相比，验证的是训练过程中四视图的变化，测试执行的是100个连续视图的预测，并最终将生成视图做成视频。return pred_rgb, pred_normal, pred_mask, None
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        for data in loader:
            ...
            self.local_step += 1
            self.global_step += 1
            # 可以设置mask_stop_step < iters, 就不会使用默认视图
            use_default_views = self.global_step % self.opt.known_view_interval == 0 and self.global_step <= self.opt.mask_stop_step
            datas = [data]
            if use_default_views:
                datas = self.default_view_data
            for data_i in datas:
                self.optimizer.zero_grad()
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    ...
                    pred_rgbs, pred_depths, loss = self.train_step(use_default_views, data_i, save_guidance_path=save_guidance_path)
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth, preds_mask, loss = self.eval_step(data)  # 验证时的数据应该是固定的视角和方位每次都是那8个, depth 被改成了normal
        ...
        if self.epoch == self.opt.update_mask_epoch:
            self.update_mask()

    def update_mask(self):
        print('---------------开始更新Mask,epoch={}---------------'.format(self.epoch))
        print('---------------开始更新Mask,epoch={}---------------'.format(self.epoch))
        print('---------------开始更新Mask,epoch={}---------------'.format(self.epoch))
        epoch = self.opt.update_mask_epoch
        workspace = self.workspace
        C1 = self.opt.mask_image.split('/')[-1]
        self.old_mask_image = self.opt.mask_image
        self.opt.mask_image = os.path.join(workspace, 'validation', C1)

        print('---------------self.opt.mask_image={}---------------'.format(self.opt.mask_image))
        print('---------------self.opt.mask_image={}---------------'.format(self.opt.mask_image))
        print('---------------self.opt.mask_image={}---------------'.format(self.opt.mask_image))

        self.opt.mask_stop_step = self.opt.mask_use_step
        print('---------------mask_stop_step=400000---------------')
        print('---------------mask_stop_step=400000---------------')
        print('---------------mask_stop_step=400000---------------')
        for i in range(8):
            path = os.path.join(workspace, 'validation', 'df_ep00{}_000{}_mask.png'.format(epoch, i+1))
            smoothed_image = remove_jagged_edges(path)
            im = Image.fromarray(smoothed_image)

            # os.rename(self.opt.mask_image + '-{}.png'.format(i*45), self.opt.mask_image + 'up-{}.png'.format(i*45))

            im.save(os.path.join(workspace, 'validation', "{}-{}.png".format(C1, i*45)))
            print(os.path.join(workspace, 'validation', "{}-{}.png".format(C1, i*45)))
    ...
    def save_checkpoint(self, name=None, full=False, best=False):
        ...
        if not best:
            ...
            if len(self.stats["checkpoints"]) > self.max_keep_ckpt:
                ...
                if self.epoch != 40 and self.epoch != 100:
