...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, light_d=None, ambient_ratio=1.0, shading='albedo', bg_color=0, perturb=False,
                 T_thresh=1e-4, binarize=False, normal=False, **kwargs):
        # rays_o, rays_d: [B, N, 3]
        # return: image: [B, N, 3], depth: [B, N]
        prefix = rays_o.shape[:-1]
        rays_o = rays_o.contiguous().view(-1, 3)
        ...
        if not torch.is_tensor(bg_color) and bg_color != 1:
            # a, b, c = 183, 169, 158
            a, b, c = 255*0.6, 255*0.6, 255*0.6
            a, b, c = 167, 212, 216
            a, b, c = 231, 242, 226
            res = [a/255, b/255, c/255]

            res1 = torch.full((640000, 3), res[0],device=device)
            res1[:, 1] = res[1]
            res1[:, 2] = res[2]
            bg_color = res1
